...
def test_no_new_or_vanished_legacy_checks(config_check_info):
    ...
    assert not new_legacy_checks, "Found these new legacy checks: %s. Implementing new legacy " \
                                  "checks is forbidden, please use the new API." \
                                  % ', '.join(sorted(new_legacy_checks))

    vanished_legacy_checks = expected_legacy_checks - current_legacy_checks
    assert not vanished_legacy_checks, "The following legacy checks have vanished: %s. Please " \
