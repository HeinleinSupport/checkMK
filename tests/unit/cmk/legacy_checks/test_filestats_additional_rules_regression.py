...
def test_check_filestats_additional_rules_regression_remaining_files(
    ...
    remaining_msg = [r for r in result if "Remaining files: 2" in r[1]]
    ...
    apport_log_msg = [r for r in result if "/var/log/apport.log.2.gz" in r[1]]
...
def test_check_filestats_additional_rules_regression_file_details(
    ...
    remaining_files_detail = [r for r in result if "/var/log/boot.log" in r[1] and "Age:" in r[1]]
...
def test_check_filestats_additional_rules_regression_age_thresholds(
    ...
    newest_msg = [r for r in result if "Newest:" in r[1]]
    assert len(newest_msg) >= 2  # Should have overall and sys-specific messages

    oldest_msg = [r for r in result if "Oldest:" in r[1]]
