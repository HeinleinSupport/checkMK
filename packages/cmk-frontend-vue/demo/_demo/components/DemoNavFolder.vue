...
<template>
  <button
    class="demo-nav-folder"
    :class="{ 'demo-nav-folder--root': isRoot }"
  ...
  <ul v-if="isOpen" class="demo-nav-folder__children">
    <li v-for="child in folder.children" :key="child.path">
      <DemoNavFolder v-if="child.type === 'folder'" :folder="child" />
      <DemoNavPage v-else :page="child" />
    </li>
  </ul>
</template>
...
.demo-nav-folder--root {
  font-size: 14px;
  padding: 8px 0 8px 12px;
}

.demo-nav-folder__children {
  margin-left: 20px;
  list-style: none;
  padding: 0;
  margin-top: 4px;
