...
def test_parse_esx_vsphere_heartbeat() -> None:
    parsed_section = esx_vsphere_vm.parse_esx_vm_heartbeat_status(
...
def testcheck_heartbeat(vm_status: str, check_state: State) -> None:
    section = SectionESXVm(
        ...
        cpu=None,
        datastores=(),
        heartbeat=HeartBeat(status=HeartBeatStatus(vm_status.upper()), value=vm_status),
        host=None,
        name=None,
        systime=None,
    )
    check_result = list(
        esx_vsphere_vm_heartbeat.check_heartbeat(
            esx_vsphere_vm_heartbeat.CHECK_DEFAULT_PARAMETERS, section
        )
    )
...
def testcheck_heartbeat_gray() -> None:
    section = SectionESXVm(
        ...
        cpu=None,
        datastores=(),
        heartbeat=HeartBeat(status=HeartBeatStatus.GRAY, value="gray"),
        host=None,
        name=None,
        systime=None,
    )
...
def testcheck_heartbeat_params(vm_status: HeartBeatStatus, params_key: str) -> None:
    section = SectionESXVm(
        ...
        cpu=None,
        datastores=(),
        heartbeat=HeartBeat(status=vm_status, value=""),
        host=None,
        name=None,
        systime=None,
    )

    check_result = list(esx_vsphere_vm_heartbeat.check_heartbeat({params_key: 0}, section))
    results = [r for r in check_result if isinstance(r, Result)]
    assert len(results) == 1
